...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # Validate API providers - log which are active/missing
    try:
        from app.core.api_providers import get_provider_manager
        provider_mgr = get_provider_manager()
        provider_mgr.log_status()
    except Exception as exc:
        logger.warning("api_provider_validation_failed", error=str(exc))
    ...
    # Start new unified market broadcaster (WebSocket updates every 3 seconds)
    market_broadcaster = None
    try:
        from app.services.market import get_market_broadcaster
        market_broadcaster = get_market_broadcaster()
        await market_broadcaster.start()
        logger.info("market_broadcaster_started")
    except Exception as exc:
        logger.warning("market_broadcaster_start_failed", error=str(exc))

    yield

    # Graceful shutdown
    if market_broadcaster is not None:
        try:
