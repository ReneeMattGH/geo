...
export function PortfolioPage({ email }: PortfolioPageProps) {
    ...
    // Filter assets by market category and search query
    const filteredUniverse = useMemo(() => {
        return assetUniverse.filter(a => {
            const notInHoldings = !holdings.some(h => h.symbol === a.symbol)
            const matchesSearch = 
                a.label.toLowerCase().includes(searchQuery.toLowerCase()) ||
                a.symbol.toLowerCase().includes(searchQuery.toLowerCase())
            const matchesMarket = selectedMarket === 'all' || a.asset_class === selectedMarket
            return notInHoldings && matchesSearch && matchesMarket
        })
            ...
            <section>
                ...
                <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
                    {marketCategories.map(cat => (
                        <button
                            key={cat.key}
                            onClick={() => setSelectedMarket(cat.key)}
                            className={cn(
                                'px-3 py-1.5 rounded-lg text-[10px] font-semibold whitespace-nowrap transition-all border',
                                selectedMarket === cat.key
                ...
                <input
                    ...
                    className="w-full bg-[#07091a]/80 border border-gray-700/50 rounded-lg px-3 py-2 text-[11px] text-white placeholder-gray-600 focus:outline-none focus:border-white/30 mb-3"
                />

                {marketsLoading ? (
                    <div className="flex items-center justify-center py-8 text-gray-600 text-[11px]">
                        <RefreshCw className="h-3.5 w-3.5 animate-spin mr-2" /> Loading markets…
                    </div>
                ) : (
                    <div className="space-y-4">
                        {Object.entries(groupedAssets).map(([category, assets]) => {
                            ...
                            return (
                                <div key={category}>
                                    <p className="text-[9px] uppercase tracking-widest text-gray-600 mb-2 flex items-center gap-2">
                                    ...
                                    <div className="space-y-1">
                                        {assets.map(asset => (
                                            ...
                                            >
                                                <div className="flex items-center gap-2 min-w-0">
                                                    <span className="text-white/70 font-bold text-[11px] group-hover:text-white transition-colors shrink-0">{asset.symbol}</span>
                                                    <span className="text-gray-500 text-[11px] truncate">{asset.label}</span>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <Plus className="h-3.5 w-3.5 text-gray-600 group-hover:text-white transition-colors shrink-0" />
                                                </div>
                                            </button>
                                        ))}
                                    </div>
